...
var state = CM["@codemirror/state"];
var language = CM["@codemirror/language"];
var view = CM["@codemirror/view"];
var search = CM["@codemirror/search"];
var commands = CM["@codemirror/commands"];
...
function initVim(CodeMirror) {
  ...
  "Left|Right|Up|Down|End|Home"
    .split("|")
    .concat(Object.keys(specialKeys))
    .forEach(function (x) {
      vimToCmKeyMap[(specialKeys[x] || "").toLowerCase()] = vimToCmKeyMap[
        x.toLowerCase()
      ] = x;
    });
  ...
  function commandMatches(keys, keyMap, context, inputState) {
    ...
    for (var i = startIndex; i < keyMap.length; i++) {
      ...
      if (
        ...
        (command.context == "operatorPending"
          ? !operatorPending
...
class CodeMirror {
  ...
  foldCode(pos) {
    ...
    let tmpRanges = state.EditorSelection.create(
      [state.EditorSelection.range(index, index)],
      0
    ).ranges;
  ...
  setSelections(p, primIndex) {
    var doc = this.cm6.state.doc;
    var ranges = p.map((x) => {
      return state.EditorSelection.range(
  ...
  getSearchCursor(query, pos) {
    ...
    function rCursor(doc, from = 0, to = doc.length) {
      return new search.RegExpCursor(
        doc,
        source,
  ...
  findPosV(start, amount, unit, goalColumn) {
    ...
    let range = state.EditorSelection.range(
      startOffset,
      startOffset,
      goalColumn
    );
  ...
  getOption(name) {
    switch (name) {
      ...
      case "indentWithTabs":
        return this.cm6.state.facet(language.indentUnit) == "\t"; // TODO
      case "indentUnit":
        return this.cm6.state.facet(language.indentUnit).length || 2;
      // for tests
      case "keyMap":
  ...
  forEachSelection(command) {
    ...
    this.virtualSelection = state.EditorSelection.create(
      selection.ranges,
      selection.mainIndex
    );
...
CodeMirror.commands = {
  cursorCharLeft: function (cm) {
    commands.cursorCharLeft(cm.cm6);
  },
  redo: function (cm) {
...
const hideNativeSelection = state.Prec.highest(
  view.EditorView.theme(themeSpec)
);
function getBase(view$1) {
  ...
  let left =
    view$1.textDirection == view.Direction.LTR
      ? rect.left
      : rect.right - view$1.scrollDOM.clientWidth;
  return {
    left: left - view$1.scrollDOM.scrollLeft,
    top: rect.top - view$1.scrollDOM.scrollTop,
  };
}
...
const vimPlugin = view.ViewPlugin.fromClass(
  class {
    constructor(view$1) {
      this.status = "";
      this.query = null;
    ...
    highlight(query) {
      this.query = query;
      if (!query) return (this.decorations = view.Decoration.none);
      let { view: view$1 } = this;
      let builder = new state.RangeSetBuilder();
      for (
        let i = 0, ranges = view$1.visibleRanges, l = ranges.length;
        i < l;
        i++
      ) {
